    ...
    while (1) {
        if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
            ...
            if (foundIdx != -1) {
                ...
                strncpy(uiPayload.name, database[foundIdx].name.c_str(), 31);
                ...
                strncpy(mqttPayload.name, database[foundIdx].name.c_str(), 31);
            ...
            xQueueSend(mqttTxQueue, &mqttPayload, portMAX_DELAY);
            xQueueSend(displayQueue, &uiPayload, portMAX_DELAY);
